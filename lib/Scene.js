...
var _vissense = require('vissense');
...
var Scene = function () {
    function Scene(id, opts) {
        ...
        this.vissence = (0, _vissense2.default)(this.canvas);
    ...
    _createClass(Scene, [{
        ...
        key: '_onFullyVisible',
        value: function _onFullyVisible() {
        ...
        value: function _initElementContainer() {
            var canvas_width = parseInt(this.$canvas.css('width'));
            var canvas_height = parseInt(this.$canvas.css('height'));
        ...
        key: '_resetElementContainer',
        value: function _resetElementContainer() {
            var canvas_width = parseInt(this.$canvas.css('width')) + parseInt(this.$canvas.css('border-left-width')) + parseInt(this.$canvas.css('border-right-width'));
            var canvas_height = parseInt(this.$canvas.css('height')) + parseInt(this.$canvas.css('border-top-width')) + parseInt(this.$canvas.css('border-bottom-width'));
            var border_top_width = parseInt(this.$canvas.css('border-top-width'));
            var border_right_width = parseInt(this.$canvas.css('border-right-width'));
            var border_bottom_width = parseInt(this.$canvas.css('border-bottom-width'));
            var border_left_width = parseInt(this.$canvas.css('border-left-width'));
        ...
        key: '_hiddenElementContainer',
        value: function _hiddenElementContainer() {
            this.elementContainer.css({
                display: 'none'
            });
            this.visibleState = false;
        ...
        key: '_reversePos',
        value: function _reversePos(realX, realY) {
            var x = (realX - this.originOffsetX) * this.initZoom / this.zoom;
            var y = (realY - this.originOffsetY) * this.initZoom / this.zoom;
            return [x, y];
        }
    }, {
        key: '_reverseOffset',
        value: function _reverseOffset(x, y) {
            var realX = x * this.zoom / this.initZoom + this.originOffsetX;
            var realY = y * this.zoom / this.initZoom + this.originOffsetY;
        ...
        value: function _adjustZoomOffset(originX, originY, zoomDelta) {
            var _zoomOffset2 = this._zoomOffset(originX, originY, this.originOffsetX, this.originOffsetY, zoomDelta),
            ...
            _lodash2.default.forEach(this.boxs, function (box, handle) {
                var _zoomOffset4 = this._zoomOffset(originX, originY, box.x + box.offsetX, box.y + box.offsetY, zoomDelta),
                    _zoomOffset5 = _slicedToArray(_zoomOffset4, 2),
                    offsetX = _zoomOffset5[0],
                    offsetY = _zoomOffset5[1];
        ...
        value: function _onMouseWheel(e) {
            ...
            var zoomDelta = 1 + wheelDelta / 120 / 20;
            this.zoom *= zoomDelta;
        ...
        value: function setZoom() {
            ...
            var originX = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
            var originY = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;

            var zoomDelta = zoom / this.zoom;
            this.zoom = zoom;
